...
def get_post(id: int): # fastapi does the validation for us, informative error message
    ...
    print(post) 
    # post = find_post(int(id)) # Convert it to an integer manually
    post = find_post(id)
...
def delete_post(id:int):
    ...
    # my_posts.pop(index)
    index = find_index_post(id) # Call function
    ...
    my_posts.pop(index)
    # return {'message': 'post was successfully deleted'} # will get error 
...
def update_post(id: int, post: Post):
    ...
    # Convert data we receive from front end -> python dict
    post_dict = post.dict()
    # Set id = the index of that post
    post_dict['id'] = id
    # Update the post in dict (of that index) with our new post
